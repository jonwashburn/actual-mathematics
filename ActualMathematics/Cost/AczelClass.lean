...
namespace ActualMathematics.Cost
...
`ActualMathematics.Cost.AczelProof`.
...
A concrete `instance` is provided in `ActualMathematics.Cost.AczelProof`
via the unconditional `dAlembert_contDiff_top` theorem in
`ActualMathematics.Cost.AczelTheorem`. -/
